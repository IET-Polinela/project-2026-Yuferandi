...
//
// SPA_URL: Alamat di mana SPA Citizen Portal di-serve. Jika tidak ada local
//          web server, Playwright akan membuka file HTML langsung dari disk.
//          Gunakan environment variable PLAYWRIGHT_SPA_URL untuk override.
//
// CATATAN PENTING :
//   - file:// URL akan bekerja tanpa server jika semua asset JS/CSS ada di disk
//     dan SPA tidak memerlukan CORS untuk local file access.
//   - Untuk frontend yang membutuhkan web server, jalankan `python -m http.server`
//     atau `npx http-server` di folder root SPA lalu set PLAYWRIGHT_SPA_URL.
// ---------------------------------------------------------------------------
const BASE_URL = 'http://localhost:8000';
const DEFAULT_SPA_FILE = path.resolve(__dirname, '..', 'index.html').replace(/\\/g, '/');
const SPA_URL = process.env.PLAYWRIGHT_SPA_URL || `file://${DEFAULT_SPA_FILE}`;
 ...
 *
 * Menggunakan pola wildcard "** /api/**" (dua bintang lalu slash api slash dua bintang),
 * fungsi ini akan mencegat request ke domain apapun
