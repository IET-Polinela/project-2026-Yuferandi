...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        payload = {
            ...
            'description': 'Lampu jalan di depan sekolah mati sejak seminggu lalu.',
            'location': 'Jl. Merdeka',
        }
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Report.objects.filter(title='Lampu Jalan Rusak').exists())
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        payload = {
            'title': '',
            'category': 'Infrastruktur',
            'description': 'Laporan tanpa judul yang seharusnya ditolak.',
            'location': 'Jl. Melati',
        }
        response = self.client.post(url, payload, format='json')
